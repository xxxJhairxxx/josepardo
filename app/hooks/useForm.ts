...
export const useForm = (
  ...
  const validateRegexMessage = (type: string): string => {
    switch (type) {
      ...
      case 'carrera':
        return 'Please enter a valid career name.'
      case 'phone':
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
        axios.post(
            ...
            {
                data: {nombres:formState.name,apellidos:formState.last_name,carrera_interes: formState.carrera, celular:formState.phone, email:formState.email},
            }
        )
        .then((response) => {
